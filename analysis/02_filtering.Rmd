...
analysis_inputs <- resolve_analysis_inputs()

dir.create("intermediates", showWarnings = FALSE, recursive = TRUE)
dir.create("relevant_rds", showWarnings = FALSE, recursive = TRUE)

# Archived-original mode recreates both named coverage checkpoints without
# overwriting a locally rebuilt POST_sites.rds.
if (analysis_inputs$mode == "archived") {
  saveRDS(
    analysis_inputs$dataset_presence_sites,
    "intermediates/POST_sites_18of23_datasets.rds"
  )
  saveRDS(
    analysis_inputs$filtered_sites,
...
all_results <- extractResults(
  analysis_inputs$matrices[["all_mean"]],
  analysis_inputs$matrices[["all_sd"]],
  0.5
)
male_results <- extractResultsParallel(
  analysis_inputs$matrices[["male_mean"]],
  analysis_inputs$matrices[["male_sd"]],
  0.5
)
female_results <- extractResultsParallel(
  analysis_inputs$matrices[["female_mean"]],
  analysis_inputs$matrices[["female_sd"]],
  0.5
)
...
# If a window has fewer than 100 strict hits, extra_filter applies the original
# fallback: pass the SD rule, keep the top 1,000 absolute correlations, then
# retain the 100 largest beta changes.
...
filtered_list <- analysis_inputs$filtered_sites

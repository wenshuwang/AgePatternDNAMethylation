...
```{r}
# Keep Illumina cg probes plus the few historical ch probes used by Hannum and
# Shireby, while removing headers/intercepts. This also makes the imports
# tolerant of supplementary files that name their ID column differently (for
# example, McEwen uses "ID" in the public file).
extract_cpg_ids <- function(values) {
  values <- trimws(as.character(unlist(values, use.names = FALSE)))
  unique(values[grepl("^(cg[0-9]{8}|ch\\.)", values)])
}

# Get clock sites
Hannum_CpGs <- read_excel("data/ref/CpGsToInvestigate/hannum_cpgs.xlsx")
Hannum_CpGs <- extract_cpg_ids(Hannum_CpGs$Marker) #71
Levine_CpGs <- read.csv("data/ref/CpGsToInvestigate/levine_cpgs.csv", stringsAsFactors=FALSE)
Levine_CpGs <- Levine_CpGs[-1, ]
Levine_CpGs <- extract_cpg_ids(Levine_CpGs$CpG) #513
...
Horvath_CpGs <- extract_cpg_ids(Horvath_CpGs[, 1]) #353
McEwen_CpGs <- read.csv("data/ref/CpGsToInvestigate/mcewen_cpgs.csv")
McEwen_id_column <- intersect(c("CPG", "CpG", "ID", "Probe"), names(McEwen_CpGs))[1]
if (is.na(McEwen_id_column)) {
  stop("Could not identify the CpG ID column in mcewen_cpgs.csv.")
}
McEwen_CpGs <- extract_cpg_ids(McEwen_CpGs[[McEwen_id_column]]) #94
Wu_CpGs <- read_excel("data/ref/CpGsToInvestigate/aging-11-102399-s003..xlsx")
Wu_CpGs <- Wu_CpGs[-1, ]
Wu_CpGs <- extract_cpg_ids(Wu_CpGs$CpGs) #111
Belsky_CpGs <- getRequiredProbes(backgroundList=FALSE)
Belsky_CpGs <- extract_cpg_ids(Belsky_CpGs) #173
load("data/ref/CpGsToInvestigate/epitoc.Rd")
Teschendorff_CpGs <- as.data.frame(dataETOC2.l[1])
Teschendorff_CpGs <- extract_cpg_ids(rownames(Teschendorff_CpGs)) #163
Shireby_CpGs <- readLines("data/ref/CpGsToInvestigate/CorticalClockCoefs.txt")[-1]
Shireby_CpGs <- sapply(strsplit(Shireby_CpGs, " "), `[`, 1)
Shireby_CpGs <- extract_cpg_ids(Shireby_CpGs) #347
Lu2_raw <- read_excel(
  "data/ref/CpGsToInvestigate/lu2.xlsx",
  col_names = FALSE
)
Lu2_CpGs <- extract_cpg_ids(Lu2_raw[[1]]) #140
clock_cpgs <- list(
  McEwen = McEwen_CpGs,
  Wu = Wu_CpGs,
  Hannum = Hannum_CpGs,
...
mainPipeline <- function(num_clusters, save=FALSE, clock=FALSE, gender_state="all", ret, labs) {
  ...
  # macOS-only workaround for forked parallel workers that initialize Objective-C.
  # It is inert on Windows/Linux but retained for reproducibility on the original setup.
  ...
  if (gender_state=="all") {
    ...
    stop("Invalid gender state: ", gender_state)
  }
  
  # Keep the two analyses disjoint: clock=TRUE analyzes only clock CpGs; the
  # aaCpG analysis explicitly removes clock CpGs from its cohort-specific set.
  ...
  # Build the clock-membership annotation in the same input row order as B.
  # ComplexHeatmap will then reorder the heatmap and annotation together. This
  # removes the old dependency on a separately saved correct_order.rds file.
  if (clock) {
    clock_annotation_matrix <- vapply(
      clock_cpgs,
      function(clock_set) as.integer(rownames(B) %in% clock_set),
      integer(nrow(B))
    )
    rownames(clock_annotation_matrix) <- rownames(B)
    clock_annotation_df <- as.data.frame(clock_annotation_matrix)
    clock_annotation_df[] <- lapply(
      clock_annotation_df,
    ...
    ha_local <- rowAnnotation(
      df = clock_annotation_df,
      col = clock_colors,
  ...
  # Z-score each CpG across ages so clustering reflects trajectory shape rather
  # than the CpG's absolute methylation level.
  B <- t(scale(t(B)))
  B <- as.matrix(B)
  ...
  # Fill sparse missing ages by linear interpolation within each CpG. rule=2 in
  # interpolate_na carries the nearest observed value beyond either endpoint.
  ...
  if (clock == TRUE) {
      ...
      ht <- Heatmap(B, name = " ", 
          row_km = num_clusters,
          row_km_repeats = 10,  # repeat k-means starts to stabilize assignments
  ...
  if (save) {
    ...
    ggsave(
      file.path(output_dir, paste0(gender_state, clock, num_clusters, "_combined_plot.pdf")),
      device = cairo_pdf,
      plot = combined_plot,
      width = 15,
      height = 6,
  ...
  if (ret=="df") {
    ...
    return(list(data = B_with_clusters, plot = combined_plot))
  } else {

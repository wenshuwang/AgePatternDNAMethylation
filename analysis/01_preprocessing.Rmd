...
```{r}
# Resume dependency: valid_samples.rds is created near the end of this notebook.
# On a fresh run, process the datasets once to create that checkpoint; on later
# runs it prevents loading samples with missing ages or ages above the cutoff.
...
# The manuscript reports the number of CpGs present in at least 18 of the
# 23 datasets. Save that dataset-coverage result separately so it is not
# confused with the stricter sample-coverage list used downstream.
dataset_presence_sites <- rownames(running_site_list)[
  which(!is.na(running_site_list$Count) & running_site_list$Count >= 18)
]
saveRDS(
...
# Reproduce the original downstream analysis: retain CpGs measured in at
# least 4,176 of the 4,641 valid samples (90%). Matrix generation and the
# filtering analysis continue to read this list from POST_sites.rds.
analysis_sites <- rownames(site_list_by_samples)[
  which(!is.na(site_list_by_samples$Count) & site_list_by_samples$Count >= 4176)
]
saveRDS(
  analysis_sites,

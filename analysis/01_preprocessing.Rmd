...
# Recount sample coverage from the written PP files using only the 4,641
# eligible samples. This keeps invalid/over-age sample columns from inflating
# the 4,176-sample checkpoint and independently verifies the PP outputs.
...
# The lightweight source-matrix scan is run by run_preprocessing.R before this
# notebook. Its dataset-presence counts must agree with the full raw-data run.
...
if (!file.exists(independent_running_path)) {
  stop("Missing independent 18-of-23 presence checkpoint: ", independent_running_path)
}
independent_running <- readRDS(independent_running_path)
raw_18of23 <- rownames(running_site_list)[
  which(!is.na(running_site_list$Count) & running_site_list$Count >= 18L)
]
independent_18of23 <- rownames(independent_running)[
  which(!is.na(independent_running$Count) & independent_running$Count >= 18L)
]
if (!setequal(raw_18of23, independent_18of23)) {
  stop(
    "The full raw preprocessing and independent source-matrix scan disagree ",
    "at the 18-of-23 CpG checkpoint."
  )
}
running_site_list <- independent_running

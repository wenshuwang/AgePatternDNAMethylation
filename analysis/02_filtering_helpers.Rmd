...
# extra_filter=TRUE retains 100 candidates from the correlation/SD-filtered set,
# ranked by absolute beta change. This keeps sparse windows visible in plots.
  ...
  for (i in seq(1, ncol(results), by=3)) {
    ...
    if (extra_filter) {
      if (dim(second_sites)[1] < 100) {
        # Rank only the CpGs that passed the correlation/SD filter. The previous
        # code ranked this smaller table with positions from the full results
        # table, so changing matrix row order could change which 100 CpGs were
        # selected. CpG ID is an explicit tie-breaker for reproducible output.
        ranked_index <- order(
          -first_sites[, i+1],
          rownames(first_sites),
          na.last = NA
        )
        keep_n <- min(100L, length(ranked_index))
        second_sites <- first_sites[ranked_index[seq_len(keep_n)], , drop=FALSE]
      }
    }

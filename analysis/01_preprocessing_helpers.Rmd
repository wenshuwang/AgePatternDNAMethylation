...
# Each split contains one age row plus a subset of CpG rows, with samples in
# columns. Splitting bounds memory use while preserving all eligible samples.
...
# Rows are CpGs and columns are integer ages 0-80. average=TRUE computes the
# mean beta at each age; average=FALSE computes the within-age SD.
...
generateMatrix <- function(input_folder, cpg_list, full_metadata, gender, average, output_folder) {
  ...
  for (i in 1:length(list_files)) {
    ...
    # Reload the shared eligibility checkpoint so every split uses the same cohort.
...
```{r make-vertical-splits, eval=FALSE, purl=FALSE}
